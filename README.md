...
| **YouTube Feed 字幕/ASR 完成后自动打标** ★ | 本地 `ContentStore.update_transcript` | `finalize_youtube_feed_task` → `tag_feed_content_task` | `subscription_content_tags.sector / tickers / concepts / summary` |
...
> ★ Feed 字幕入库后本地 Worker 会自动发出 `finalize_youtube_feed_task` 回调到远程 `file_processing` 队列，触发 AI 打标 + 摘要，写入 `subscription_content_tags`（详见"远程回调任务"一节）。  
...
│  │               │     │        │     │                   │    │
│  │ ┌───────────┐ │     │        │     │ subscription_*    │    │
│  │ │ /upload-  │ │ ──► │        │     │ subscription_     │    │
│  │ │  youtube  │ │     │        │     │   content_tags ★  │    │
│  │ └───────────┘ │     │        │     │ file_processing_* │    │
│  │ ┌───────────┐ │     │        │     │ processing_stats  │    │
│  │ │ Beat /    │ │ ──► │        │     └──────────────────┘    │
│  │ │ fetch_all │ │     │        │                               │
│  │ └───────────┘ │     │        │      ▲              ▲         │
│  │               │     │        │      │ finalize_     │ finalize│
│  │ file_processing◄────┼────────┼──────┘ youtube_file  │ youtube │
│  │  ai-worker    │     │        │        task          │ _feed   │
│  │  tag_feed_    │     │        │     (Upload Link)    │ _task★  │
│  │  content_task │     │        │                      │         │
│  │     │         │     │        │                      │         │
│  │     ▼         │     │        │                      │         │
│  │  subscription_│     │        │                      │         │
...
│  │           Upload Link → signal_upload_file_ready ──────────┐│ │
│  │                                                            ││ │
...
Feed 字幕/ASR 完成后的打标回调 (★ 新增):
  本地 Worker update_transcript() 成功后
    → _signal_feed_transcript_ready(content_db_id)
    → send_task('finalize_youtube_feed_task', queue='file_processing')
    → 远程 ai-worker 消费 → trigger_auto_feed_tagging([content_id])
    → tag_feed_content_task (qwen3-max) → UPSERT subscription_content_tags
  失败静默 — 打标是增值能力，不阻断字幕入库主流程
...
本 Worker 同时服务两条业务链路，字幕降级 / ASR / 长短队列路由的逻辑是共享的，区别在**落库目标**与**完成回调**：

- **Feed 订阅链路**：`fetch_all_youtube_subscriptions` / `fetch_youtube_subscription` → 字幕 fallback → 必要时 `transcribe_youtube_feed_task` → 写 `subscription_content.transcript` → 回调远程 `finalize_youtube_feed_task` → `tag_feed_content_task` 生成 AI 标签 + 摘要写入 `subscription_content_tags`。
- **Upload YouTube Link 链路**：`fetch_youtube_file_transcript_task` → 字幕 fallback → 必要时 `transcribe_youtube_file_asr_task` → 写 `file_processing_details` → 回调远程 `finalize_youtube_file_task` 触发 auto-tag / auto-name / auto-classify。
...
flowchart TD
    ...
    %% ── Phase 3 新增: 字幕/ASR 落库后自动触发 Feeds AI 打标 ──
    J    --> SIGNAL_API["📡 signal_feed_transcript_ready<br/>→ 远程 finalize_youtube_feed_task"]
    J2   --> SIGNAL_YTDLP["📡 signal_feed_transcript_ready<br/>→ 远程 finalize_youtube_feed_task"]
    SHORT_SAVE --> SIGNAL_SHORT["📡 signal_feed_transcript_ready<br/>→ 远程 finalize_youtube_feed_task"]
    LONG_SAVE  --> SIGNAL_LONG["📡 signal_feed_transcript_ready<br/>→ 远程 finalize_youtube_feed_task"]

    SIGNAL_API   --> AUTOTAG["tag_feed_content_task<br/>(queue: file_processing, qwen3-max)<br/>→ UPSERT subscription_content_tags<br/>(sector / tickers / concepts / summary)"]
    SIGNAL_YTDLP --> AUTOTAG
    SIGNAL_SHORT --> AUTOTAG
    SIGNAL_LONG  --> AUTOTAG
    ...
    classDef autotag fill:#ecfdf5,stroke:#059669,color:#064e3b;
    ...
    class SIGNAL_API,SIGNAL_YTDLP,SIGNAL_SHORT,SIGNAL_LONG signal;
    class AUTOTAG autotag;
```
...
| **[Feed]** 字幕/ASR 写库后触发打标回调 ★ | `youtube_fetching` / `youtube_transcription*` | `signal_feed_transcript_ready → finalize_youtube_feed_task: content=x` |
| **[Feed]** 打标回调派发失败（静默，不影响主流程）| `youtube_fetching` / `youtube_transcription*` | `signal_feed_transcript_ready 派发失败 (不阻断主流程): content=x error=…` |
...
本地 Worker 完成字幕/ASR 写库后，通过两种不同的回调通知远程主站，分别触发对应的后处理管线。

#### Feed 订阅链路的打标回调 ★（v2026-04 新增）

Feed 字幕成功写入 `subscription_content` 后，`ContentStore.update_transcript` 在内部调用：

```python
ContentStore._signal_feed_transcript_ready(content_db_id)
    → app.send_task('finalize_youtube_feed_task', args=(content_db_id,), queue='file_processing')
```

远程 `ai-worker` 消费 `finalize_youtube_feed_task` 后：

1. 调用 `trigger_auto_feed_tagging([content_id])`；
2. 派发 `tag_feed_content_task`（queue=`file_processing`，qwen3-max）；
3. `FeedContentTaggingService.tag_content` → `content_tag_feeds_v1` prompt → UPSERT `subscription_content_tags`（sector / industry / tickers / concepts / confidence / summary）。

**失败静默**：`_signal_feed_transcript_ready` 内部有 `try/except`，`send_task` 失败只写 warning 日志，不影响字幕已落库的事实。漏打标的内容可用运维脚本补跑：

```bash
# 在主项目远程服务器上补回填
python scripts/maintenance/backfill_feed_tags.py --since-hours 24
```
...
2. 仅在 `outcome='completed'` 时触发 auto-tag / auto-name / auto-classify（写入 `file_tags`）。
...
> 本节描述的降级链路被 **Feed 订阅**和 **Upload YouTube Link** 两条业务共用。两条链路的**落库目标**与**完成回调**不同：
...
```
── [1/26] 频道: Bloomberg Television ──
  ✓ 字幕获取成功: video=FLmZ6HnOSkE lang=en 字数=5230
  signal_feed_transcript_ready → finalize_youtube_feed_task: content=1009214

# 远程 ai-worker 日志 (celery_ai_worker.log):
[finalize_feed] content_id=1009214 triggering auto-feed-tagging
已触发 Feeds 自动打标: content_id=1009214, delay=2s
开始 feed 打标任务xxx, content_id=1009214
调用 LLM 打标: content_id=1009214 channel=youtube text_len=1951
✅ LLM分析完成: 总时间=4.15s tokens=2575 费用=0.025750 RMB
Feeds 打标完成: content_id=1009214 channel=youtube sector=信息技术 tickers=2 concepts=5 summary_len=76 耗时=4.18s
```
...
# ── v2026-04 新增: Feed 字幕落库后的打标回调 ──
'finalize_youtube_feed_task': {'queue': 'file_processing'},              # Feed 字幕完成回调（远程，触发 AI 打标）
'tag_feed_content': {'queue': 'file_processing'},                        # Feed 单条内容 AI 打标 + 摘要
'batch_tag_feed_content': {'queue': 'file_processing'},                  # Feed 批量打标派发器
```
...
> `git subtree` 会自动将子目录的提交历史完整同步到 `LocalVirtualService` 仓库，无需手动维护两份代码。
